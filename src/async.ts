...
export const tryAsync = <A>(tryRun: TryAsyncTask<A>): Fx<Async & Fail, A> =>
  doFx(function* () {
...
export const delay = (ms: number): Fx<Delay & Async, void> =>
  doFx(function* () {
...
const delayFail = (ms: number): Fx<Delay & Async & Fail, never> =>
  doFx(function* () {
    yield* delay(ms)
...
export const race = <C1 extends Async, C2 extends Async, A, B, Fxs extends readonly Fx<any, any>[]>(
  fx1: Fx<C1, A>,
  fx2: Fx<C2, B>,
  ...fxs: Fxs
): Fx<C1 & C2 & AllEffects<Fxs>, A | B | AnyResult<Fxs>> => raceArray([fx1, fx2, ...fxs])
